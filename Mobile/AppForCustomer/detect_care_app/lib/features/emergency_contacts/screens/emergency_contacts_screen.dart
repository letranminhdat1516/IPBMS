  ...
  void _showAddContactModal() {
    showModalBottomSheet(
      ...
      builder: (context) => AddContactModal(
        levels: () {
          final all = const ['Ưu tiên cấp 1', 'Ưu tiên cấp 2'];
          final used = _contacts.map((c) => c.level).toSet();
          return all.where((l) => !used.contains(l)).toList();
        }(),
        onSubmit: (dto) async {
          final userId = context.read<AuthProvider>().currentUserId;
          if (userId == null || userId.isEmpty) return;
            ...
            if (context.mounted) {
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Đã thêm liên hệ')),
                );
              }
            }
          } catch (e) {
  ...
  void _showEditContactModal(UiEmergencyContact c) {
    showModalBottomSheet(
      ...
      builder: (context) => EditContactModal(
        contact: c,
        // compute available levels for edit: allow the contact's own level plus any not used by others
        levels: () {
          final all = const ['Ưu tiên cấp 1', 'Ưu tiên cấp 2'];
          final usedByOthers = _contacts
              .where((x) => x.id != c.id)
              .map((x) => x.level)
              .toSet();
          final res = all
              .where((l) => !usedByOthers.contains(l) || l == c.level)
              .toList();
          // ensure current level present
          if (!res.contains(c.level)) res.insert(0, c.level);
          return res;
        }(),
        onUpdate: (dto) async {
          final userId = context.read<AuthProvider>().currentUserId;
  ...
  Widget _buildContactsSection() {
    return Column(
      ...
      children: [
        ...
        const Text(
          'Quản lý thông tin liên lạc của những người thân cận đáng tin cậy nhận được cảnh báo.',
          style: TextStyle(
            fontSize: 14,
            color: Color(0xFF64748B),
            fontWeight: FontWeight.w500,
...
class _AddContactModalState extends State<AddContactModal>
  ...
  void initState() {
    ...
    if (widget.levels.isNotEmpty) _level = widget.levels.first;
  }
