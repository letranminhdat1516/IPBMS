import 'package:detect_care_app/features/search/models/search_models.dart';
import 'package:detect_care_app/features/search/providers/search_provider.dart';
...
class _AdvancedSearchScreenState extends State<AdvancedSearchScreen> {
  ...
  Future<void> _executeQuickAction(String action, SearchResult result) async {
    ...
    if (success && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Thực hiện thao tác thất bại')),
      );
    }
  }
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Tìm sự kiện, người chăm sóc, hóa đơn...',
    ...
    switch (entityType) {
      case 'events':
        return [
          ...
          const PopupMenuItem(value: 'escalate', child: Text('Tăng mức')),
        ];
      case 'invoices':
        return [
          const PopupMenuItem(
  ...
  void _navigateToDetail(SearchResult result) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Đi tới chi tiết ${result.entityType}')),
    );
  }
