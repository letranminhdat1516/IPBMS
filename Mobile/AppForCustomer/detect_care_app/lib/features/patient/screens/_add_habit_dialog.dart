        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: ListTile(
                      ...
                      title: Text('Giờ bắt đầu (sleep_start)'),
                      subtitle: Text(_sleepStart ?? 'Chưa đặt'),
                      trailing: TextButton(
                        onPressed: () async {
                          final t = await _pickTime(null);
                          if (t != null) {
                            setState(() {
                              _sleepStart =
                  ...
                  Expanded(
                    child: ListTile(
                      contentPadding: EdgeInsets.zero,
                      title: Text('Giờ kết thúc (sleep_end)'),
                      subtitle: Text(_sleepEnd ?? 'Chưa đặt'),
                      trailing: TextButton(
                        onPressed: () async {
                          final t = await _pickTime(null);
                          if (t != null) {
              ...
              TextFormField(
                decoration: InputDecoration(
                  labelText: 'Ghi chú (text hoặc JSON object)',
                ),
        ...
        ElevatedButton(
          onPressed: () {
            if (_formKey.currentState?.validate() ?? false) {
              dynamic parsedNotes = _notes;
              if (_notes != null && _notes!.isNotEmpty) {
                try {
                  parsedNotes = jsonDecode(_notes!);
                } catch (_) {
                  parsedNotes = _notes;
                }
              }
