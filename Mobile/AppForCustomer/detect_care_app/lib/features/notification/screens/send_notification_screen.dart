...
class _SendNotificationScreenState extends State<SendNotificationScreen> {
  ...
  void initState() {
    super.initState();
    print('🔔 [SendNotificationScreen] initState called');
  ...
  Future<void> _loadAcceptedCustomers() async {
    print('🔔 [SendNotificationScreen] _loadAcceptedCustomers starting');
    try {
      ...
      final isCustomer = (currentUser?.role ?? '').toLowerCase() == 'customer';

      for (final a in list) {
        if (a.status?.toLowerCase() == 'accepted' && a.isActive) {
          if (isCustomer) {
            if (!seen.contains(a.caregiverId)) {
              seen.add(a.caregiverId);
              options.add(
                _CustomerOption(
                  id: a.caregiverId,
                  name: a.caregiverName ?? 'Người chăm sóc ${a.caregiverId}',
                ),
              );
            }
          } else {
      ...
      try {
        print(
          '🔔 [SendNotificationScreen] listPending returned ${list.length} items',
        );
        if (list.isNotEmpty) {
          print('🔔 [SendNotificationScreen] sample item: ${list.first}');
        }
        final mapped = options.map((o) => '${o.id}:${o.name}').toList();
        print(
          '🔔 [SendNotificationScreen] Dropdown options (${mapped.length}): $mapped',
        );
      } catch (e, st) {
        print('🔔 [SendNotificationScreen] Debug print failed: $e');
        print(st.toString());
      }

      setState(() {
        _customers = options;
        _sendLog = 'Loaded ${options.length} recipients';
      });
    } catch (e) {
      print('🔔 [SendNotificationScreen] Error loading customers: $e');
      setState(() => _sendLog = 'Lỗi tải người nhận: $e');
    }
  }
  ...
  Future<void> _onSendFcm(BuildContext context, auth.User currentUser) async {
    ...
    try {
      // Determine messaging direction and participants based on user role
      final String direction;
      final String fromUserId;
      final List<String> toUserIds;

      print('🔄 [SendNotificationScreen] Determining messaging direction...');

      if (currentUser.role.toLowerCase() == 'caregiver') {
        print(
          '👨‍⚕️ [SendNotificationScreen] User is CAREGIVER - sending to customers',
        );
        // Caregiver sending to customers
        ...
        if (_selectedReceiver == '_ALL_') {
          ...
          toUserIds = list
              ...
              .toSet()
              .toList();
          print('✅ [SendNotificationScreen] Filtered customers: $toUserIds');
        } else {
          toUserIds = [_selectedReceiver];
          print('🎯 [SendNotificationScreen] Single receiver: $toUserIds');
        }
      } else {
        print(
          '👤 [SendNotificationScreen] User is CUSTOMER - sending to caregivers',
        );
        // Customer sending to their assigned caregivers
        ...
        if (_selectedReceiver != '_ALL_') {
          toUserIds = [_selectedReceiver];
          print(
            '🎯 [SendNotificationScreen] Single caregiver selected: $toUserIds',
          );
        } else {
          final list = await _assignDs.listPending();
          print(
      ...
      if (mounted) {
        scaffoldMessenger.showSnackBar(
          SnackBar(content: Text('Gửi: $ok · Lỗi: $fail')),
        );
      }
    } catch (e) {
