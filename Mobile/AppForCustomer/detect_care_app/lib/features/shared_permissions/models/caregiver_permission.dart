  ...
  factory CaregiverPermission.fromJson(Map<String, dynamic> json) {
    ...
    for (final k in boolKeys) {
      if (json.containsKey(k)) {
        ...
        if (json.containsKey(alt)) permissionsJson[k] = json[alt];
      }
    }

    // Numeric / array keys
    if (json.containsKey('log_access_days')) {
      permissionsJson['log_access_days'] = json['log_access_days'];
    } else if (json.containsKey('log:access_days')) {
      permissionsJson['log_access_days'] = json['log:access_days'];
    }

    if (json.containsKey('report_access_days')) {
      permissionsJson['report_access_days'] = json['report_access_days'];
    } else if (json.containsKey('report:access_days')) {
      permissionsJson['report_access_days'] = json['report:access_days'];
    }

    if (json.containsKey('notification_channel')) {
      permissionsJson['notification_channel'] = json['notification_channel'];
    } else if (json.containsKey('notification:channel')) {
      permissionsJson['notification_channel'] = json['notification:channel'];
    }
    ...
    return CaregiverPermission(
      ...
      caregiverPhone: () {
        ...
        return caregiverObj?['phone']?.toString() ??
            json['caregiver_phone']?.toString();
      }(),
      caregiverFullName: () {
        final caregiverObj = json['caregiver'] as Map<String, dynamic>?;
        final top = json['caregiver_full_name'];
        if (caregiverObj != null) {
          final nested =
              caregiverObj['full_name'] ??
              caregiverObj['name'] ??
              caregiverObj['fullName'];
          if (nested != null && nested.toString().trim().isNotEmpty)
            return nested.toString();
        }
        if (top != null && top.toString().isNotEmpty) return top.toString();
        return json['caregiver_name']?.toString() ?? '';
      }(),
