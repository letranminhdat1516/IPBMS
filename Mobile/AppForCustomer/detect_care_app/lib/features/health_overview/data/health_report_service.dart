...
class HealthReportRemoteDataSource {
  ...
  // Helper: build extra headers (X-User-Id) and append user_id query param
  Future<(String, Map<String, String>?)> _buildPathAndHeaders(
    ...
    // If AuthStorage doesn't have userId yet (timing or race condition),
    // attempt to decode it from the access token as a fallback.
      ...
      try {
        ...
        if (token != null && token.isNotEmpty) {
          ...
          if (candidate != null && candidate.isNotEmpty) {
            userId = candidate;
            // store into in-memory cache for future fast reads (do not persist)
            // We avoid calling saveAuthResult here because we don't have full userJson.
            // This mirrors AuthStorage behavior but keeps side-effects minimal.
            // ignore: invalid_use_of_protected_member
            // ... no-op: only update internal cache via AuthStorage's private cache is not accessible.
          }
        }
      } catch (e) {
        // swallow and continue; this is only a best-effort fallback for diagnostics
  ...
  // Try decode a JWT (no verification) and return payload as Map
  Map<String, dynamic> _tryDecodeJwtPayload(String token) {
    try {
      final parts = token.split('.');
      if (parts.length < 2) return {};
      final payload = parts[1];
      // Base64 decode (handle URL-safe base64 and padding)
  ...
  Future<HealthReportOverviewDto> overview({
    ...
    // Debug: print raw high_risk_time for diagnosis (only in debug builds)
    try {
      // ignore: avoid_print
      debugPrint(
